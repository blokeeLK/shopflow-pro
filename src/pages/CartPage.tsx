    ...
    <div className="container py-6 md:py-10">
      ...
      <div className="bg-card rounded-lg border p-4 mb-6">
        {remainingForFree > 0 ? (
          ...
          <p className="text-xs text-success font-semibold mb-2">🎉 Você ganhou frete grátis!</p>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {items.map((item) => {
            ...
            return (
              <div key={`${item.product.id}-${item.size}`} className="bg-card rounded-lg border p-4 flex gap-4">
                <Link to={`/produto/${item.product.slug}`} className="w-20 h-24 bg-secondary rounded-md overflow-hidden flex-shrink-0">
                  <img src={item.product.images[0]} alt={item.product.name} className="w-full h-full object-cover" />
                </Link>
                <div className="flex-1 min-w-0">
                  <Link to={`/produto/${item.product.slug}`} className="font-display font-semibold text-sm text-foreground line-clamp-1 hover:underline">
                    {item.product.name}
                  </Link>
                  <p className="text-xs text-muted-foreground mt-0.5">Tamanho: {item.size}</p>
                  <p className="font-display font-bold text-foreground mt-1">{formatCurrency(price)}</p>
        ...
        <div className="bg-card rounded-lg border p-6 h-fit sticky top-20">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-muted-foreground">
              <span>Frete</span>
              <span>{remainingForFree <= 0 ? "Grátis" : "Calcular no checkout"}</span>
            </div>
          </div>
          ...
          <button className="w-full mt-4 bg-accent text-accent-foreground font-display font-semibold py-3.5 rounded-lg hover:bg-accent/90 transition-colors flex items-center justify-center gap-2 text-sm">
