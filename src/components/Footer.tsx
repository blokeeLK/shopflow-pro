    ...
    <footer className="bg-[#1a1a2e] text-white mt-16">
      <div className="container py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            <h4 className="font-display font-semibold text-sm uppercase tracking-wider mb-4">Categorias</h4>
            <div className="flex flex-col gap-2">
              {categories.map((cat) => (
                <Link key={cat.slug} to={`/categoria/${cat.slug}`} className="text-sm hover:opacity-80 transition-opacity">{cat.name}</Link>
              ))}
            </div>
          </div>
          <div>
            <h4 className="font-display font-semibold text-sm uppercase tracking-wider mb-4">Atendimento</h4>
          ...
          <div>
            <h4 className="font-display font-semibold text-sm uppercase tracking-wider mb-4">Sede</h4>
            <p className="text-sm leading-relaxed whitespace-pre-line">
              {settings?.company_address || "Endereço não configurado"}
            </p>
            <Link to="/faq" className="text-sm hover:opacity-80 transition-opacity mt-3 inline-block">
